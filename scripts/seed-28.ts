...
async function main() {
  console.log("=== 28차 시드 사전 검증 (6-check) ===");
  ...
  const SPORT_KEYWORDS: Record<number, string[]> = {
    1: ["보디빌딩", "헬스", "포징", "벤치", "데드", "스쿼트", "PT", "트레이너", "근육", "EPOC", "ATP", "포즈", "피지크", "구술", "시험복", "포스파겐"],
  };
  for (const p of newPosts) {
  ...
  console.log(`✅ 체크 2: 카테고리 매핑 정확 (보디빌딩=1)`);
  ...
  // 체크 4: 22-23차 보디빌딩(문구류) + 27차 작성자 풀과 충돌 검증
  const PREV_BB_POOL = [
  ...
  for (const a of authorSet) {
    if (PREV_BB_POOL.includes(a)) throw new Error(`작성자가 22-23차 풀과 겹침: ${a}`);
    if (PREV_27_AUTHORS.includes(a)) throw new Error(`작성자가 27차 풀과 겹침: ${a}`);
  }
  for (const m of MIXED_POOL) {
    if (PREV_BB_POOL.includes(m)) throw new Error(`MIXED_POOL이 22-23차 풀과 겹침: ${m}`);
    if (PREV_27_AUTHORS.includes(m)) throw new Error(`MIXED_POOL이 27차 작성자 풀과 겹침: ${m}`);
  }
  console.log(`✅ 체크 4: 22-23차/27차 풀과 충돌 없음`);

  // 체크 6: region 형식 (pickRegion이 REGION_GROUPS 기반)
  ...
  console.log(`✅ 체크 6: region 형식 "광역시도 - 시군구" 정상 (e.g. "${testRegion}")`);
