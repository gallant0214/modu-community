...
import { newPosts, pickRegion, PREV_AUTHOR_POOLS } from "../app/api/admin/seed-community-reviews-59/data";
...
function check2_CategoryMatch() {
  const BB_KEYWORDS = [
    "보디빌딩","포징","포즈","쿼터턴","피지크","클래식","실기","구술","연수","연수원","현장실습","가채점",
    "감독관","심사","커트라인","합격","도핑","금지약물","베타차단제","근육","신전","외전","근섬유","단면적",
    "척추기립근","삼각근","광배근","글루카곤","인슐린","오버트레이닝","로우","킥백","덤벨","바벨","헬스장",
    "뼈","관절","연골","인대","힘줄","자비스트","트레이너","점수","규정","생체","자격증","공단","시급",
    "안전관리","시간강사","라운드","회전근개","오십견","노인",
  ];
  const SWIM_KEYWORDS = [
    "수영","수영장","강사","텃세","레인","수모","마스터즈","철인","IM","안전요원","실기","구술","인프라","회원",
  ];
  for (const p of newPosts) {
    if (p.categoryId !== 1 && p.categoryId !== 5) {
      throw new Error(`체크2 실패 - cat이 1/5가 아님: ${p.title} (cat=${p.categoryId})`);
    }
    const text = p.title + p.content + p.comments.map((c) => c.content).join(" ");
    const kws = p.categoryId === 5 ? SWIM_KEYWORDS : BB_KEYWORDS;
    if (!kws.some((k) => text.includes(k))) {
      console.warn(`⚠️ 체크2 경고 - cat=${p.categoryId} 키워드 없음: ${p.title}`);
    }
  }
  const c1 = newPosts.filter((p) => p.categoryId === 1).length;
  const c5 = newPosts.filter((p) => p.categoryId === 5).length;
  console.log(`✅ 체크2 통과 — cat=1 보디빌딩 ${c1}개 / cat=5 수영 ${c5}개, 키워드 검증`);
}
...
async function main() {
  console.log(`\n=== 59차 시드 ${DRY_RUN ? "[DRY RUN]" : "[PROD INSERT]"} ===\n`);
