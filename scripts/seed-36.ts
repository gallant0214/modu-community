/**
 * 36차 시드 prod insert 스크립트
 *
 * 실행: cd modu-community && npx -y tsx scripts/seed-36.ts [--dry-run]
 *
...
function check2_CategoryMatch(posts: SeedPost[]) {
  ...
  for (const p of posts) {
    ...
    if (p.categoryId === 5) {
      const kws = SPORT_KEYWORDS[5];
      const hasMatch = kws.some((k) => text.includes(k));
      if (!hasMatch) {
        throw new Error(`카테고리 5번(수영) 키워드 없음: "${p.title}"`);
      }
    }
  }
...
function runAllChecks() {
  ...
  console.log(
    `\n📦 시드 데이터 요약: 게시글 ${newPosts.length}개 + 댓글 ${totalComments}개 (전부 categoryId=5 수영)\n`
  );
}
