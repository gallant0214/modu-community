// 33차 시드: 수영 12 게시글 (전부 수영)
// 카톡 출처: 2026-05-16~19 생활체육스포츠지도사 2급 수영 (시험 신청 5/18 당일 토픽 다수)
// 6-check + 누적 풀(22-23/27/28/29/30/31/32차) 검증 후 insert
...
const PREV_31_AUTHORS = [
  "도토리키움", "빨래걷기실패", "늦잠전문가", "야식러버사", "김치찌개팬2",
  "명상초보러2", "사무실너구리", "다육식물러2", "떡볶이덕후2", "야경수집가",
  "라면취향러", "분리수거왕",
];
const PREV_32_AUTHORS = [
  "곶감홀릭", "뜨개질대장", "오믈렛장인", "만두피전문", "해바라기씨",
  "단호박러버", "감자조림팬", "모닝커피러", "시금치무침", "숙면예찬가",
  "딸기잼매니아", "주말산책러",
];
...
function validate() {
  ...
  const SPORT: Record<number, string[]> = {
    5: ["수영", "다이빙", "배영", "자유형", "글라이딩", "인터벌", "IM", "스트로크", "킥", "물속", "풀", "사이드해", "롤오버", "백해", "조명", "lux", "수심", "레인", "SM", "청주체대", "영법", "벽 터치", "호흡", "턴", "응시", "시험복", "레이싱복", "옥련LNG", "25m", "50m", "다이어트", "오버트레이닝", "롤링", "구술", "면접", "사이드킥", "발차기", "신청", "추가취득", "일반과정", "등급", "운동량", "기록", "휴식", "신경계 피로", "회복"],
  };
  for (const p of newPosts) {
...
const REGION_WEIGHTS: Record<string, number> = {
  ...
  광주광역시: 2, 울산광역시: 2, 세종특별자치시: 1, 강원도: 2, 충청북도: 2,
  충청남도: 2, 전라북도: 2, 전라남도: 2, 경상북도: 2, 경상남도: 2, 제주특별자치도: 1,
};
...
async function run() {
  ...
  if (dryRun) {
    console.log("🔍 dry-run 모드: insert 건너뜀");
    let totalComments = 0;
    for (const p of newPosts) totalComments += p.comments.length;
    console.log(`   posts=${newPosts.length}, comments=${totalComments}`);
    console.log("   region 예시 3개:");
    for (let i = 0; i < 3; i++) console.log("    -", pickRegion());
    return;
  }
  ...
  console.log(
    `\n✅ 33차 완료: postsInserted=${postsInserted}, commentsInserted=${commentsInserted}`
  );
