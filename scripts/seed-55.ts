...
import { newPosts, pickRegion, PREV_AUTHOR_POOLS } from "../app/api/admin/seed-community-reviews-55/data";
...
function check2_CategoryMatch() {
  const SWIM_KEYWORDS = [
    "수영","수영장","수영복","수영강사","강습","실기","구술","장체","생체","노인","장애인","유소년",
    "뇌병변","지적장애","자폐","척수","IM","돌핀","접영","평영","자유형","배영","턴","스타트","잠영",
    "데크","벽터치","수온","하이폭식","심판","경영","계영","혼계영","부정출발","로프","스윔오프",
    "월드아쿠아틱스","대한수영연맹","WPS","WA","IPC","DQ","실격","수험번호","수험표","원주","청주","홍성",
    "영운","드림파크","젖산","완영","완주","청심환","도핑","계측","염분","레인","웜업","수구","인명구조",
    "복장","개인짐","감독관","심사관","면접관","고사장","오전","오후","등급분류","매트","조파","항력",
    "필기","합격","연수","자격증","라이프가드","최대산소섭취량","FITT","트레이닝","TUE","커트라인",
  ];
  for (const p of newPosts) {
    if (p.categoryId !== 5) {
...
// check3b — 라이브 DB의 모든 시드 닉네임(베이스)과 대조 (병렬 차수 풀 누락 방지)
...
async function main() {
  console.log(`\n=== 55차 시드 ${DRY_RUN ? "[DRY RUN]" : "[PROD INSERT]"} ===\n`);
