/**
 * 39차 시드 prod insert 스크립트
 *
 * 실행: cd modu-community && npx -y tsx scripts/seed-39.ts [--dry-run]
 */
...
function check2_CategoryMatch(posts: SeedPost[]) {
  ...
  for (const p of posts) {
    ...
    if (p.categoryId === 5) {
      const hasMatch = SWIM_KEYWORDS.some((k) => text.includes(k));
      if (!hasMatch) {
