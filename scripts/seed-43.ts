...
function check2_CategoryMatch() {
  const BB_KEYWORDS = [
    "보디빌딩","보디빌더","보디","피지크","사프","사피","사이드피지크","사이드프지크",
    "쿼터턴","규정포즈","포징","포즈","무대","컨벤셔널","데드","데드리프트","벤치","스쿼트",
    "사이드","트라이","프론트","랫","어브도미널","근비대","ATP","ATP-PC","글리코겐",
    "BMI","체질량","체지방","피하지방","내장지방","정지방","글루카곤","인슐린",
    "에피네프린","아드레날린","교감신경","WADA","KADA","도핑","TUE","마스킹","이뇨제","흥분제",
    "IFBB","무대 규격","시상면","관상면","횡단면","시상축","관상축","비타민 B6","피리독신",
    "흉쇄유돌근","SCM","카르보넨","워밍업","좌심실","HDL","LDL","CPR","AED","PRICE","PRICES",
    "ACL","PCL","MCL","LCL","반월상","무릎 인대","BCAA","아미노산","단백질","크레아틴",
    "그렐린","멜라토닌","성장호르몬","근육","근비대","체급","비키니","클래식보디빌딩","보드피트",
    "여자비키니","사프","사피","보드피트니스","유산소","무산소","해당","젖산","에너지 시스템",
    "선수 심장","Athletes Heart","청심환","발살바","락아웃","흡기","호기","트레이닝","FITT",
    "Rep","Set","워밍업","쿨다운","Rest","휴식","근육 점성","관절","가동성",
  ];
  for (const p of newPosts) {
    if (p.categoryId !== 1) {
...
function check6_RegionFormat() {
  // pickRegion이 "광역시도 - 시군구" 형식 반환하는지 샘플 확인
  for (let i = 0; i < 5; i++) {
    const r = pickRegion();
    if (!r.includes(" - ")) {
...
// ============= 메인 =============
async function main() {
  console.log(`\n=== 43차 시드 ${DRY_RUN ? "[DRY RUN]" : "[PROD INSERT]"} ===\n`);
  ...
  console.log(`post id range: ${insertedIds[0]} ~ ${insertedIds[insertedIds.length - 1]}`);
}
