// 32차 시드: 수영 9 + 보디빌딩 3 = 12 게시글
// 카톡 출처: 2026-05-13~19 스포츠지도사 실기 생체/장체/노인/유소년
// 6-check + 누적 풀(22-23/27/28/29/30/31차) 검증 후 insert
...
function validate() {
  ...
  const SPORT: Record<number, string[]> = {
    1: ["보디빌딩", "사이드체스트", "포징", "포즈", "자비스트", "회전근개", "어깨", "헬스", "추가취득", "특별과정"],
    5: ["수영", "다이빙", "배영", "자유형", "글라이딩", "인터벌", "IM", "스트로크", "킥", "물속", "풀", "장체 수영", "장애인 수영", "사이드해", "롤오버", "크로스오버", "백해", "조명", "lux", "수심", "레인", "SwimCoach", "SM", "청주체대", "영법", "벽 터치", "호흡", "턴"],
  };
  for (const p of newPosts) {
...
// ========== 지역 가중치 픽 ==========
// region-data.ts 의 실제 광역시도 명칭에 맞춰 키 작성 (강원도/전라북도, 특별자치도 아님)
...
async function run() {
  ...
  console.log(
    `\n✅ 32차 완료: postsInserted=${postsInserted}, commentsInserted=${commentsInserted}`
  );
