...
/**
 * 주민번호 원문 → AES-256-GCM 암호화 바이너리
 * 형식: iv(12 bytes) || ciphertext || authTag(16 bytes)
 * 형식이 13자리가 아니거나 키가 없으면 null.
 */
export function residentEncrypt(raw: string | null | undefined): Buffer | null {
  const d = digitsOnly(raw);
  if (d.length !== 13) return null;
...
/**
 * AES-256-GCM 암호화 바이너리 → 주민번호 원문
 * 실패(키 없음/데이터 손상) 시 null.
 */
export function residentDecrypt(blob: Buffer | Uint8Array | null | undefined): string | null {
  if (!blob) return null;
  const key = getResidentEncKey();
  if (!key) return null;
