...
const PREFERENCES_OPTIONS = ["동종업계 경력자", "관련 자격증 소지자", "장기근무 가능자", "초보 가능", "인근 거주자", "대학생 가능", "운전 가능자", "기타"];
const BENEFITS_OPTIONS = ["4대보험", "인센티브", "식대지원", "회원권 제공", "교육 지원", "퇴직금", "기타"];

function splitWithOther(csv: string, options: string[]) {
  if (!csv) return { selected: [] as string[], other: "" };
...
function joinWithOther(selected: string[], other: string) {
  return selected.map(s => (s === "기타" && other.trim() ? other.trim() : s)).join(", ");
}
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const sb = splitWithOther(data.benefits || "", BENEFITS_OPTIONS);
        setBenefits(sb.selected);
        setBenefitOther(sb.other);
        const sp = splitWithOther(data.preferences || "", PREFERENCES_OPTIONS);
        setPreferences(sp.selected);
        setPreferenceOther(sp.other);
        ...
        setSalaryMin(parsedSalary.salaryMin);
        setSalaryMax(parsedSalary.salaryMax);
        setSalaryDirectSub(parsedSalary.salaryDirectSub);
    ...
    try {
      ...
      const res = await fetch(`/api/jobs/${jobId}`, {
        ...
        body: JSON.stringify({
          ...
          benefits: joinWithOther(benefits, benefitOther),
          preferences: joinWithOther(preferences, preferenceOther),
  ...
  return (
    <div className="min-h-screen bg-[#F8F4EC] dark:bg-zinc-950 pb-28">
      ...
      <div className="mx-auto max-w-2xl px-4 sm:px-6 py-6 space-y-5">
        ...
        <Section number={5} title="추가 정보" subtitle="선택 입력 — 공고의 매력도를 높여줍니다">
          <Field label="우대 조건" hint="선택">
            <SelectButton value={preferences.length ? joinWithOther(preferences, preferenceOther) : ""} placeholder="해당하는 조건을 선택하세요" onClick={() => setShowPreferences(true)} />
          </Field>

          <Field label="복리후생" hint="선택">
            <SelectButton value={benefits.length ? joinWithOther(benefits, benefitOther) : ""} placeholder="제공하는 복리후생을 선택하세요" onClick={() => setShowBenefits(true)} />
          </Field>
        </Section>
      ...
      <Modal open={showPreferences} onClose={() => setShowPreferences(false)} title="우대 조건"
        ...
        <div>
          {PREFERENCES_OPTIONS.map(o => (
            <CheckItem key={o} label={o} checked={preferences.includes(o)} onToggle={() => {
              setPreferences(prev => {
                const next = prev.includes(o) ? prev.filter(x => x !== o) : [...prev, o];
                if (o === "기타" && prev.includes(o)) setPreferenceOther("");
                return next;
              });
            }} />
          ))}
          {preferences.includes("기타") && (
            <div className="px-5 py-4 border-t border-[#E8E0D0]/70 dark:border-zinc-800">
              <input type="text" value={preferenceOther} onChange={e => setPreferenceOther(e.target.value)}
                placeholder="기타 항목을 입력하세요" className={inputCls} maxLength={50} />
            </div>
          )}
        </div>
      </Modal>
      ...
      <Modal open={showBenefits} onClose={() => setShowBenefits(false)} title="복리후생"
        ...
        <div>
          {BENEFITS_OPTIONS.map(o => (
            <CheckItem key={o} label={o} checked={benefits.includes(o)} onToggle={() => {
              setBenefits(prev => {
                const next = prev.includes(o) ? prev.filter(x => x !== o) : [...prev, o];
                if (o === "기타" && prev.includes(o)) setBenefitOther("");
                return next;
              });
            }} />
          ))}
          {benefits.includes("기타") && (
            <div className="px-5 py-4 border-t border-[#E8E0D0]/70 dark:border-zinc-800">
              <input type="text" value={benefitOther} onChange={e => setBenefitOther(e.target.value)}
                placeholder="기타 항목을 입력하세요" className={inputCls} maxLength={50} />
            </div>
          )}
        </div>
