...
const SPORTS_WORDS = /태권도|유도|검도|복싱|권투|합기도|주짓수|킥복싱|무에타이|무술|헬스|피트니스|트레이너|크로스핏|필라테스|요가|발레|에어로빅|무용|줄넘기|수영|골프|테니스|배드민턴|탁구|축구|풋살|농구|배구|야구|클라이밍|암벽|볼더링|승마|체조|댄스|양궁|펜싱|사격|스키|스노보드|스케이트|볼링|스쿼시|스포츠|체육|운동|지도사|코치|강사|인스트럭터|레저|생활체육|체력단련|PT|GYM|짐|웰니스|피지컬|유아체육|유소년|어린이체육|장애인체육|장애인스포츠|노인체육|노인스포츠|실버체육|호텔피트니스|호텔스파|운동처방|체육교사|스포츠강사|국민체육|체육회|체육진흥|종합운동장|문화체육|시설관리공단/i;
const SPORTS_IND = /태권도|무술|체력단련|수영장|골프장|골프연습장|스포츠|체육|운동|경기용|레크리에이션|댄스|교습학원|스포츠 교육|스포츠 서비스/;

// 제조·산업 업종 — "도장" 이 등장해도 여기 걸리면 페인트 직종으로 판단
const INDUSTRIAL_IND = /제조업|기계|금속|화학|플라스틱|반도체|전자부품|변압기|자동차|운송|물류|일반목적용|산업용|조립|건설|창호|토공사|도금|성형|가공|수리업|단조|적층|표면처리|오락용품 제조|인력 공급|인력공급|인사관리/;

// 확실히 스포츠와 무관한 제목 패턴 (도장공사, 생산직 등)
// 공백 허용(\s*) 으로 "자동차 도장사원" 같은 공백 포함 표현도 매칭.
const EXCLUDE_TITLE = /도장\s*공|도장\s*사원|도장\s*기사|도장\s*작업|도장\s*공사|도장\s*부|도장\s*파트|도장\s*업체|도장\s*협력|도장\s*반|도장\s*팀|도장\s*과|도장\s*직|도장\s*과장|전착\s*도장|분체\s*도장|자동차\s*도장|스프레이\s*도장|우레탄\s*도장|선박\s*도장|차체\s*도장|에폭시\s*도장|하지\s*엔지니어|생산직|생산팀|생산부|생산관리|생산보조|제조직|조립직|조립|용접|프레스|사출|금형|CNC|선반|포장|상하차|지게차|화물|배송기사|납품기사|운전기사|택배/i;
