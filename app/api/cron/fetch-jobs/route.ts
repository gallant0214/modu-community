...
// "유도" 는 한글 lookbehind 로 단어 시작에서만 매치
//   - "유도복/유도장/유도부": 통과 (앞에 한글 X)
//   - "교통유도/유도등/유도탄/유도분만": 차단 (앞에 한글)
// "스키" 는 lookahead 로 비-스포츠 합성어 차단
//   - "스키복/스키장/스키부/스키화/스키어": 통과
//   - "스키드(skid)/스키니(skinny)/스키마(schema)/스키핑(skipping)/스키터": 차단
// NOTE: '시설관리공단' 단독은 SPORTS_WORDS 에서 제외 — 중랑구시설관리공단 같은 일반 시설관리공단이
//       사무·돌봄 직군을 모집해도 회사명만으로 통과되던 사례 차단. 체육시설관리공단은 '체육'으로 매칭됨.
