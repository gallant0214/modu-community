...
const INDUSTRIAL_IND = /제조업|기계|금속|화학|플라스틱|반도체|전자부품|변압기|자동차|운송|물류|일반목적용|산업용|조립|건설|창호|토공사|도금|성형|가공|수리업|단조|적층|표면처리|오락용품 제조/;

// 확실히 스포츠와 무관한 제목 패턴 (도장공사, 생산직 등)
// 공백 허용(\s*) 으로 "자동차 도장사원" 같은 공백 포함 표현도 매칭.
const EXCLUDE_TITLE = /도장\s*공|도장\s*사원|도장\s*기사|도장\s*작업|도장\s*공사|도장\s*부|도장\s*파트|도장\s*업체|도장\s*협력|분체\s*도장|자동차\s*도장|스프레이\s*도장|우레탄\s*도장|선박\s*도장|차체\s*도장|하지\s*엔지니어|생산직|생산팀|생산부|생산관리|생산보조|제조직|조립직|조립|용접|프레스|사출|금형|CNC|선반|포장|상하차|지게차|화물|배송기사|납품기사|운전기사|택배/i;
...
function isSportsRelated(title: string, company: string, indTpNm: string): boolean {
  ...
  // 2) 이중의미 "도장" — 업종이 제조/산업 이면 페인트 직종으로 판단해 제외
  if (/도장/.test(title) && INDUSTRIAL_IND.test(indTpNm)) return false;

  // 3) 제목/회사명에 스포츠 키워드가 있으면 통과
