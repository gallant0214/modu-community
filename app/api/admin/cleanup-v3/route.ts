...
const TITLE_BAD =
  "(바리스타|베이커리|카페\\s*직원|카페\\s*매니저|매점|미화|세차|룸메이드|하우스키핑|홀서빙|식음|외곽\\s*미화|호텔\\s*프론트|호텔\\s*객실|화장품|식품|요양보호사|방문요양|잔디\\s*관리|잔디관리|예초|클럽하우스|식당\\s*홀|레스토랑\\s*조리|주방보조|조리원|조리사|샤워실|간병|컨트리클럽.*프론트|골프.*프론트)";
