...
export async function POST(request: Request) {
  ...
  const [
    ...
    sb.from("posts").select("id, title, category_id, created_at").eq("firebase_uid", uid).order("created_at", { ascending: false }).limit(100),
    sb.from("comments").select("id, post_id, content, created_at, posts(category_id)").eq("firebase_uid", uid).order("created_at", { ascending: false }).limit(100),
    sb.from("job_posts").select("id, title, created_at, is_closed").eq("firebase_uid", uid).order("created_at", { ascending: false }).limit(100),
