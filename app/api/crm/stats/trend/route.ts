 ...
 * 최근 12개월 PT매출 추이 (대시보드용, PDF 1-1).
 *
 * 응답: [{ ym: "2026-01", revenue: 1000000 }, ...] 12개 (해당 월 발급 합계)
 *
...
export async function GET(request: Request) {
  ...
  // 12개월 윈도우
  const now = new Date();
  const months: { ym: string; start: string; end: string; revenue: number }[] = [];
  for (let i = 11; i >= 0; i--) {
    ...
    months.push({ ym, start, end, revenue: 0 });
  }
  ...
  return NextResponse.json({
    months: months.map(({ ym, revenue }) => ({ ym, revenue })),
  });
}
