...
export async function POST(request: Request) {
  ...
  if (couponIssueId) await finalizeCouponUse(couponIssueId, "membership", created.id);
  ...
  if (mileageEarned > 0 || mileageUsed > 0) {
    ...
    await supabase
      ...
      .eq("id", memberId)
      .eq("center_id", ctx.centerId);
    // 회원앱 마일리지 내역에 표시되도록 원장(crm_member_mileage_logs) 로그 남김
  ...
  // 0원(무료·서비스 발급)도 결제내역에 남긴다 — 대여권·수강권과 동일 기준.
