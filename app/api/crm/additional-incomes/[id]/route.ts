...
/** PATCH /api/crm/additional-incomes/[id] — 내용/금액/메모 수정 (admin). */
export async function PATCH(
  ...
  if (!incomeId) return NextResponse.json({ error: "잘못된 요청" }, { status: 400 });
  ...
  await supabase.from("crm_audit_logs").insert({
    ...
    actor_uid: ctx.uid,
    action: "additional_income.update",
...
/** DELETE /api/crm/additional-incomes/[id] — 삭제 (admin). */
export async function DELETE(
  ...
  await supabase.from("crm_audit_logs").insert({
    center_id: ctx.centerId,
    actor_uid: ctx.uid,
    action: "additional_income.delete",
    entity_type: "crm_additional_incomes",
    entity_id: incomeId,
    payload: {} as never,
  });
