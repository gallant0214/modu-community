...
export async function POST(request: Request) {
  ...
  const seedGrades: { center_id: number; base_role: string; label: string; is_system: boolean; sort_order: number }[] = [
    ...
    { center_id: center.id, base_role: "trainer", label: "아르바이트",   is_system: false, sort_order: 10 },
    { center_id: center.id, base_role: "trainer", label: "FC",           is_system: false, sort_order: 11 },
  ];
