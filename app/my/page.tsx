...
/* ── 설정 행 ── */
function SettingRow({ label, onClick, icon, danger, value, count }: { label: string; onClick?: () => void; icon: React.ReactNode; danger?: boolean; value?: string; count?: number }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-[#F5F0E5] dark:hover:bg-zinc-800 transition-colors border-b border-[#E8E0D0]/50 dark:border-zinc-800 last:border-0 text-left"
    >
      ...
      {typeof count === "number" ? <span className="text-xs text-[#8C8270] dark:text-zinc-500">({count})</span> : null}
                      ...
                      {list.map((t) => {
                        ...
                        return (
                          <li key={t.id} className="relative bg-[#FEFCF7] dark:bg-zinc-900 border border-[#E8E0D0] dark:border-zinc-700 rounded-2xl overflow-hidden">
                            ...
                            >
                              <div className="relative shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-[#F5F0E5] dark:bg-zinc-800 flex items-center justify-center">
                                ...
                                {/* 거래탭과 동일한 좌하단 상태 라벨 */}
                                {(isSold || isReserved) && (
                                  <span className={`absolute left-1 bottom-1 px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                    isSold ? "bg-black/65 text-white" : "bg-[#C0392B]/85 text-[#FFE082]"
                                  }`}>
                                    {isSold ? "거래완료" : "예약중"}
                                  </span>
                                )}
                            ...
                            {isMyTradesTab && (
                              <div className="flex border-t border-[#E8E0D0] dark:border-zinc-700">
                                ...
                                <button
                                  ...
                                  className="flex-1 py-3 text-[13px] font-bold text-[#2A251D] dark:text-zinc-100 hover:bg-[#FBF7EB]/50 dark:hover:bg-zinc-800/40 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                                >
                                  끌어올리기
                                </button>
                              </div>
